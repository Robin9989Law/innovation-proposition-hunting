...
DIG_TEXT = {
    ("红海", "小"): "红海，决心小：停在这里。锚点已经找到，全文 0，不绕圈，也不钻缝。",
    ("蓝海", "小"): "蓝海，决心小：全文 0。只核对锚点的题目和摘要，不绕圈。不像就收一个小题目。不许换个场景把它说成新的。",
    ("看不清", "小"): "还看不清，决心小：继续只看题目和摘要，直到找出最危险的那一篇。不要铺开领域，不要读全文。",
    ("红海", "中"): "红海，决心中：全文只读锚点这一篇，三问答完。一圈上的论文只看题目和摘要。不许换场景，也不许收成它没做的那一小块。",
    ("蓝海", "中"): "蓝海，决心中：全文只读锚点。一圈只看题目和摘要，确认圈上没有更危险的。三问答完就停。",
    ("看不清", "中"): "还看不清，决心中：先找到锚点。找到之后全文只读它，一圈只看题目和摘要。",
    ("红海", "大"): "红海，决心大：锚点加一圈里可能杀死你或露出缝的论文，读全文，六问答完就停。仍是一圈，不往外扩。不要读满 12 或 20。不许换名、换场景、收成补集。",
    ("蓝海", "大"): "蓝海，决心大：把锚点和一圈里最接近的几篇读完，六问答完，确认这片空不是没看见近邻。",
    ("看不清", "大"): "还看不清，决心大：先找到锚点，再只绕它一圈。不要用看不清当作把圈越画越大的理由。",
}
FAST_ENOUGH = (
    "先最快找到最危险的近邻，不要先凑篇数。"
    "用你要写的那句话搜，只看题目和摘要。"
    "危险是指：这篇如果成立，你的话就被占住，或能从它推出来。不是引用最多，也不是期刊最好。"
    "第一篇明显更危险的，就是锚点，先到先停。"
    "然后只绕它一圈：它直接引用的前作，和直接引用它的后续，尤其是同一批作者接着做的。"
    "不搜前作的前作。这一圈看完，判断不变，文献就够了。"
    "圈外若出现更危险的一篇，它变成新锚点，圈只重画一次。"
    "100 篇是不许再横向搜的天花板，不是要读满的数量。"
)
PAGE_QUESTIONS = {
    "ocean": {
        "type": "choice",
