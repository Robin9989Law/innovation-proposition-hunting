...
不要让它「列十个创新点」。做完应该拿到这些：

1. 研究范围已经写死，和你说的是同一块。
2. 范围里值得做的那一小块也写死了。
3. 博士论文是三个连得上、互不重复的贡献。期刊论文只有一个主贡献。
4. 每个贡献里至少有一句能被推翻、又不能靠拼旧结果得到的话。
5. 每句结论都能走回某篇论文的某一处，以及出版社或会议的正式入口。
6. 一个明白的结果：继续查、卡住了、这个想法停掉、句子可以定下来。定题之后不在这里开实验。

别人已经做过，于是停掉，也算做成了。及时停掉错方向，就是结果。
...
- 已经有的文献、草稿、实验和停掉的记录
- 你现在想先定范围，还是已经在看那一句
- 新东西从哪来、最后交什么。还没想好，就让它把选项摆出来等你选，不能它自己定

论文类型你没说清，它只能问，不能自己选博士或期刊。
...
范围可以宽，但必须是一条连得上的研究，不是一串热门词。

### 3.2 值得做的那一小块

这是范围里面你真正打算下手的一片。要写清：
...
它比范围窄，但要比单独一条定理、一个算法或一次实验宽。

### 3.3 贡献怎么分

博士论文在这一小块里放三个贡献。三个贡献至少在研究对象、主要变量、要的结果、关键证据、停的条件里有三项不一样，而且一个的结果要能接到下一个。三个标题、三套数据或三个算法步骤，不是三个贡献。

期刊论文在这一小块里只放一个主贡献。里面可以有一句主命题、跟着的结果和辅助部分。多条定理、多次实验都得服务这一个主贡献，不能拆成好几个假的主贡献。

### 3.4 准备写进论文的那一句

新不新，只判这一句。它得是这种能被打脸的话：什么条件下成立、什么条件下失效；是不是必要；边界在哪；能不能做出来；算法保证了什么；实验上什么结果会把它推翻。

「多测几个数据集」「换个场景用一下」「结果看起来很复杂」，通常都不是这一句。
...
长读之前先跑 iph judge。它说停或等人，就停。研究卡片先做快判：红海、蓝海或看不清，再等我说决心大、中、小。有 Jev 密钥再加 --jev，只给大致看法。
...
范围阶段只看题目和摘要，不读全文，也不抽结论。圈那一小块时，全文最多 12 篇，仍然不抽成条的结论。只有要判那一句时，才对选中的论文读全文、抽能改变判断的话。全文默认不超过 20 篇，结论默认不超过 60 条。超了就是 `EVIDENCE_DEPTH_EXCEEDS_LAYER`。下载了多少篇，不是进度。

用旧项目的文献名单时，旧记录都留着。另外写一份 `current_evidence_scope.json`，只列入这一轮真读过的论文和结论。不写这份，检查会按全部旧记录计数，不会因为你没写就放水。
...
来源和交付形式不是每轮都能重选的标签。你确认之后，查文献、排序、把句子定下来、做形式检查，都沿着这一条。

#### 主贡献与支持性贡献

- `PRIMARY`：论文准备当成核心创新的那一块，必须符合已经定下的交付形式
- `SUPPORTING`：用来证明、解释、划界、实现或检验主贡献的结果。可以是另一种东西，但不能变成主线

例如你选了算法优化路径，做着做着得到一条收敛界或保护定理。这些是支持性理论，可以让算法创新更站得住。程序不得转向理论创新，不能把新理论改成新的主目标，也不能用 `MIXED` 给中途换题打掩护。
...
只有用户明确确认要改主路径或主交付时，才允许重来。这不是在原流程里改一个标签，而是新一代路径：旧选择和重来的理由都留着，回到 `SCOPE_LOCK`，范围、候选、谁是主贡献、前沿文献和形式检查都重做。你没确认时，新方向只能记成支持线索，不能变成第二条主创新。
...
五个该停的地方，只说「继续」不算看过，程序不能往下走。其余步骤可以接着做，但必须：

1. 读 `workflow_state.json`；
2. 核对它说已经写好的文件是不是真的在；
3. 跑这一步该跑的检查；
4. 从 `next_required_action` 接着做；
5. 重复一遍已经定下的来源、交付形式、谁是主贡献；
6. 不把范围、路径、形式、层级、贡献编号或轮次清掉重来。

可以这样说：

```text
先运行 iph explain，用白话告诉我现在在哪。
如果到了要我看的五个地方，停下来等我自己的话。只说继续不算。
其他步骤从 next_required_action 接着做。不要重头检索，也不要改已经定下的范围。
```
...
100 篇是停搜的天花板，不是判断红海、蓝海或新不新要读满的数量。够不够，看再加一篇判断会不会变。快判和按决心停的篇数见 [给人看.md](给人看.md)。
...
1. 一上来就让它「列十个创新点」。
2. 还没有 `workflow_state.json` 就开始搜文献。
3. 把范围、那一小块、那一句混成一句「新不新」。
4. 博士论文用三个标题冒充三个贡献。
5. 期刊论文把一个主贡献拆成好几个主贡献。
6. 只存论文列表，不抽出能改变判断的那句话。
7. 结论只贴 DOI，不指回那句话。
8. 看摘要就说这篇已经覆盖了你的想法。
9. 把别人的「未来工作」翻译成自己的创新。
10. 被覆盖之后换个场景跑掉，不进那篇论文里面看它到底做到哪。
11. 上一轮抽出的结论还没处理，就继续搜新的。
12. 只用预印本宣布「已经有人做了」或「可以关了」。
13. 句子还活着，就自动开大实验。
14. 你说「继续」，它把整个流程重开一遍。
...
- 前沿查到了今年
- 最近的可靠结果、它没回答的问题、你往前的那一小步，都写准了
- 这一小步不能靠拼旧结果得到
- 有一个最小的例子、反例，或一句能被打脸的预测
- 来源和交付形式都对得上你当初确认的
- 中途冒出来的另一种结果，都标成支持，没有改主线
- 实质结论都能走回某句话
- 检查没有报错
- 写了什么情况要停
...
- 没有还没做的必要动作
- 上一轮抽出的结论都处理过了
- 交给你的说明和 `workflow_state.json` 一致
- 没有把「我们自己查过」写成「同行已经认可」

---

## 19. 先读哪一份

人先读 [给人看.md](给人看.md)。这篇教程是展开说明，术语看不懂就回那一份。

程序按这个顺序读：

1. 对人说话时读 [给人看.md](给人看.md)，执行时读 [`SKILL.md`](../SKILL.md)；
2. 要填哪一栏，再读 [`templates.md`](../templates.md) 里对应的那一节；
3. 在登记文献时读 [`evidence-pipeline.md`](../evidence-pipeline.md)；
4. 判断标准说不清时读 [`reference.md`](../reference.md)；
5. 只有旧项目已经在做实验时，才读 [`compute-funnel.md`](../compute-funnel.md)；
6. 踩过的坑在 [`case-lessons.md`](../case-lessons.md)。

不要把全部文件一次塞进上下文。做到哪一步，读那一步要的那一份。
