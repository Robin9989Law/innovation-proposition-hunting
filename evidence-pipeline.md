...
这篇规定文献名单、全文、抽出来的结论、你写出的话，这几层怎么连。程序填表时按这里。人要看流程，读 [docs/给人看.md](docs/给人看.md)。配合 [SKILL.md](SKILL.md) 使用。
...
三个全局注册表跨轮追加，不得为了满足本轮预算删除历史记录或伪改下载状态。
`current_evidence_scope.json` 是独立的预算视图：进入新碰撞时以空 scope 开始；L2
试读、K 全文和 K 原子观点每增加一项，就把对应全局 ID 加入本轮 scope。URL
完整性始终对全局文献注册表验证，证据深度只对本轮 scope 验证。
