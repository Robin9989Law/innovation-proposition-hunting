...
这篇不是定题的下一步。题目锁定并被你收下之后，这个工具就结束。实验和写论文另外做。

下面只约束两件事：定题期间允许的文献筛查、少量实例核对、探索登记；以及已经跑进实验阶段的旧项目。新课题执行 `iph advance --to COMPUTE` 必须被拒绝。
...
新课题不能从这里进入实验。下面这条命令只留给已经停在实验阶段的旧项目，不能抄去当下一步：
...
程序会先核对「还活着」和另一个检查者是否通过。授权那句话不能绕过这两条。旧项目如果真的进入了，阶段从筛查开始。确认跑完之后，先存证据，再进入计算后的句子冻结，然后换一轮检查。

实验要一档一档加，不要一上来就做完整确认。每个候选先用最便宜、最容易把它推翻的证据。多数候选应在筛查和小试验里停掉。同一轮正式确认只服务最后还活着的那一个。
...
`inventory_atoms`，并打中已声明的一条 FAIL-* 合取。改名同构或长字面量
泄漏亦不得确认。ALGORITHM 冻结句未声明合取不得确认。空清单上的多余原子
失败不算确认。终态窗口协议仍写 `NOT_YET_ACCESSED` 是常驻 INVALID。完整
硬门见 hard-gates.md。
